...
<html ${htmlAttrs}>
  ...
  function projectText(zhText, enText) {
    return getRuntimeLocale() === 'zh' ? zhText : enText;
  }

  function getCurrentConversationContext() {
    const pathname = String(location.pathname || '').replace(/\/+$/, '') || '/';
    const conversationMatch = pathname.match(/^(.*?)(?:\/c\/)([^/?#\/]+)(?:\/.*)?$/i);
    const projectHomeMatch = pathname.match(/^(.*?)(?:\/project)(?:\/.*)?$/i);
    const bareProjectMatch = pathname.match(/^(\/g\/g-p-[^/?#]+)(?:\/.*)?$/i);

    const currentConversationId = conversationMatch ? safeDecodeURIComponent(conversationMatch[2]) : '';
    const projectPathPrefix = conversationMatch
      ? (conversationMatch[1] || '')
      : projectHomeMatch
        ? (projectHomeMatch[1] || '')
        : bareProjectMatch
          ? (bareProjectMatch[1] || '')
          : '';
    const segments = projectPathPrefix.split('/').filter(Boolean);
    const projectPathSegment = segments.length ? safeDecodeURIComponent(segments[segments.length - 1]) : '';
    const projectCandidateIds = [];
    const projectMatch = projectPathSegment.match(/^(g-p-[a-z0-9]+)(?:-.+)?$/i);

    if (projectPathSegment) {
      projectCandidateIds.push(projectPathSegment);
      if (projectMatch) {
        projectCandidateIds.push(projectMatch[1]);
      }
      const genericGizmoMatch = !projectMatch ? projectPathSegment.match(/^(g-[a-z0-9]+)(?:-.+)?$/i) : null;
      if (genericGizmoMatch) {
        projectCandidateIds.push(genericGizmoMatch[1]);
      }
    }

    const isProjectPage = Boolean(projectPathPrefix && /^g-p-/i.test(projectPathSegment));

    return {
      isConversationPage: Boolean(conversationMatch),
      currentConversationId,
      projectPathPrefix,
      projectPathSegment,
      projectCandidateIds: dedupeStrings(projectCandidateIds),
      isProjectConversation: Boolean(conversationMatch && isProjectPage),
      isProjectPage,
    };
  }

  function guessProjectTitleFromSegment(segment) {
    const raw = safeDecodeURIComponent(segment).trim();
    if (!raw) return '';
    let title = raw
      .replace(/^g-p-[a-z0-9]+-?/i, '')
      .replace(/^g-[a-z0-9]+-?/i, '')
      .replace(/[-_]+/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  ...
  let projectAccessToken = null;

  async function readProjectJsonResponse(response) {
  ...
  async function getProjectAccessToken() {
    ...
    try {
      ...
      throw new Error(error?.message || projectText('获取访问令牌失败。', 'Failed to fetch the access token.'));
    }

    const data = await readProjectJsonResponse(response);
    if (!response.ok) {
      throw new Error(extractApiErrorMessage(data) || projectText('获取访问令牌失败。', 'Failed to fetch the access token.'));
    }

    const token = data?.accessToken || data?.access_token || '';
    if (!token) {
      throw new Error(projectText('未找到 accessToken。', 'No access token was found.'));
    }
  ...
  async function fetchBackendJson(path, options = {}, maxRetries = 1) {
    const url = new URL(path, location.origin).toString();
    let lastError = null;

    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
      try {
        const token = await getProjectAccessToken();
        const headers = {
          Accept: 'application/json',
        ...
        if (!response.ok) {
          const message = extractApiErrorMessage(data) || `${response.status} ${response.statusText}`.trim();
          const error = new Error(message || projectText('请求失败。', 'Request failed.'));
          error.shouldRetry = response.status === 401 || response.status === 403;
          throw error;
        }
    ...
    throw lastError || new Error(projectText('请求失败。', 'Request failed.'));
  }

  function objectContainsValue(root, needle, depth = 0, seen = new Set()) {
    if (root == null) return false;
    if (depth > 14) return false;

    if (typeof root === 'string' || typeof root === 'number') {
      return String(root) === String(needle);
    }
    ...
    if (Array.isArray(root)) {
      return root.some((value) => objectContainsValue(value, needle, depth + 1, seen));
    }

    return Object.values(root).some((value) => objectContainsValue(value, needle, depth + 1, seen));
  }

  function extractGizmoMeta(item) {
    const gizmo = item?.gizmo?.gizmo || item?.gizmo || item?.data?.gizmo?.gizmo || item?.data?.gizmo || null;
    const directId = gizmo?.id
      || item?.gizmo_id
      || item?.gizmoId
      || (typeof item?.id === 'string' && /^g(?:-p)?-[a-z0-9]/i.test(item.id) ? item.id : '');
    const directName = gizmo?.display?.name || gizmo?.name || item?.title || item?.name || '';
    return {
  ...
  function collectSidebarGizmoItems(root) {
    const entries = [];
    const seenNodes = new Set();

    const walk = (value, depth = 0) => {
      if (!value || typeof value !== 'object' || seenNodes.has(value) || depth > 16) {
        return;
      }
      seenNodes.add(value);

      const meta = extractGizmoMeta(value);
      if (meta.id) {
        entries.push({
          ...meta,
          raw: value,
  ...
  async function fetchSidebarGizmoItems() {
    const paths = [
      '/backend-api/gizmos/snorlax/sidebar?conversations_per_gizmo=100&owned_only=false',
      '/backend-api/gizmos/snorlax/sidebar?conversations_per_gizmo=100&owned_only=true',
    ];

    for (const path of paths) {
      try {
        const data = await fetchBackendJson(path, {}, 0);
        const items = collectSidebarGizmoItems(data);
        if (items.length) {
          return items;
        }
      } catch (_error) {
  ...
  function extractConversationItemsFromResponse(data) {
  ...
  function normalizeConversationListItem(item) {
    const conversation = item?.conversation || item?.data?.conversation || item?.data || item;
    const id = conversation?.id || item?.id || item?.conversation_id || item?.conversationId || '';
    if (!id) return null;

    return {
      id: String(id),
      title: String(conversation?.title || item?.title || item?.name || '').trim(),
      create_time: conversation?.create_time || conversation?.createTime || item?.create_time || item?.createTime || '',
      update_time: conversation?.update_time || conversation?.updateTime || item?.update_time || item?.updateTime || '',
    };
  }
  ...
  async function fetchProjectConversationList(gizmoId) {
    const normalizedId = String(gizmoId || '').trim();
    if (!normalizedId) {
      throw new Error(projectText('缺少 Project 标识。', 'Missing Project identifier.'));
    }
    ...
    for (let page = 0; page < 50; page += 1) {
      const data = await fetchBackendJson(`/backend-api/gizmos/${encodeURIComponent(normalizedId)}/conversations?cursor=${encodeURIComponent(cursor)}`, {}, page === 0 ? 1 : 0);
      const items = extractConversationItemsFromResponse(data);
      items.forEach((item) => {
      ...
      if (!items.length || !nextCursor || nextCursor === cursor) {
        break;
      }
      cursor = nextCursor;
      await sleep(120);
  ...
  function buildProjectConversationUrl(projectPathPrefix, conversationId) {
    return `${location.origin}${projectPathPrefix}/c/${encodeURIComponent(conversationId)}`;
  }

  function getExportContextSummary() {
    const context = getCurrentConversationContext();
    return {
      ...context,
      projectTitle: guessProjectTitleFromSegment(context.projectPathSegment),
    };
  }

  async function resolveCurrentProjectPlan() {
    const context = getCurrentConversationContext();
    if (!context.isConversationPage && !context.isProjectPage) {
      throw new Error(projectText('请先打开一个聊天或 Project 页面。', 'Open a conversation or Project page first.'));
    }
    if (!context.isProjectPage) {
      throw new Error(projectText('当前页面不在 Project 中。', 'This page is not inside a Project.'));
    }

    const guessedTitle = guessProjectTitleFromSegment(context.projectPathSegment);
    const sidebarItems = await fetchSidebarGizmoItems();
    const matchingSidebarItem = context.currentConversationId
      ? sidebarItems.find((item) => objectContainsValue(item.raw, context.currentConversationId))
      : null;
    const sidebarItemByCandidate = sidebarItems.find((item) => context.projectCandidateIds.includes(item.id));

    const candidateIds = dedupeStrings([
      matchingSidebarItem?.id,
      sidebarItemByCandidate?.id,
      ...sidebarItems
        .filter((item) => context.projectCandidateIds.some((candidate) => item.id === candidate || item.id.startsWith(`${candidate}-`)))
        .map((item) => item.id),
      ...context.projectCandidateIds,
    ]);
    ...
    for (const candidateId of candidateIds) {
      ...
      try {
        const conversations = await fetchProjectConversationList(candidateId);
        if (!conversations.length) {
          continue;
        }

        const sidebarMeta = sidebarItems.find((item) => item.id === candidateId) || matchingSidebarItem || sidebarItemByCandidate || null;
        const nextResolved = {
          gizmoId: candidateId,
          projectTitle: cleanTitle(sidebarMeta?.name) || guessedTitle || projectText('ChatGPT Project', 'ChatGPT Project'),
          conversations,
        };

        if (!fallbackResolved) {
          fallbackResolved = nextResolved;
        }
        if (!context.currentConversationId || conversations.some((item) => item.id === context.currentConversationId)) {
    ...
    if (!finalResolved) {
      throw new Error(projectText(
        '未能识别当前 Project。请先打开该 Project，再重试。',
        'Could not identify the current Project. Open the Project page and try again.'
      ));
    }

    const conversations = finalResolved.conversations.slice();
    if (context.currentConversationId && !conversations.some((item) => item.id === context.currentConversationId)) {
      conversations.push({
        id: context.currentConversationId,
        title: cleanTitle(document.title) || projectText('当前聊天', 'Current conversation'),
        create_time: '',
        update_time: '',
      });
    }
    ...
    return {
      ...context,
      gizmoId: finalResolved.gizmoId,
      projectTitle: cleanTitle(finalResolved.projectTitle) || guessedTitle || projectText('ChatGPT Project', 'ChatGPT Project'),
      projectUrl: `${location.origin}${context.projectPathPrefix}`,
      conversationCount: conversations.length,
      conversations: conversations.map((item, index) => ({
        ...item,
        index: index + 1,
        url: buildProjectConversationUrl(context.projectPathPrefix, item.id),
      })),
    };
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === 'CGPT_EXPORT_GET_CONTEXT') {
      try {
        sendResponse({ ok: true, data: getExportContextSummary() });
      } catch (error) {
        sendResponse({ ok: false, error: error?.message || projectText('读取当前页面上下文失败。', 'Failed to read the current page context.') });
      }
      return true;
    }

    if (message?.type === 'CGPT_EXPORT_LIST_CURRENT_PROJECT_CONVERSATIONS') {
      (async () => {
        try {
          const data = await resolveCurrentProjectPlan();
          sendResponse({ ok: true, data });
        } catch (error) {
          sendResponse({ ok: false, error: error?.message || projectText('读取当前 Project 失败。', 'Failed to read the current Project.') });
        }
      })();
      return true;
    }
